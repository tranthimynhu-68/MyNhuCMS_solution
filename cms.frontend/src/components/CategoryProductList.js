...
const CategoryProductList = () => {
    ...
    // 2. State quản lý loading
    const [loading, setLoading] = useState(true);

    // 3. useEffect tự động chạy khi component mount
    useEffect(() => {
        ...
        <div className="card shadow-sm border-0 rounded-lg">
            ...
            <div className="card-header bg-white border-bottom-0 pt-4 pb-2 px-4">
                <h5 className="card-title text-uppercase font-weight-bold text-dark d-flex align-items-center mb-0"
                    ...
                    <i className="fa-solid fa-cubes text-primary mr-2" style={{ fontSize: '1.3rem' }}></i>
                    Danh mục SP
                </h5>
            </div>
                        ...
                        categoryProducts.map((item) => (
                            ...
                            >
                                ...
                                <i className="fa-solid fa-chevron-right text-muted"
                                    style={{ fontSize: '0.8rem', opacity: 0.5 }}></i>
                            </button>
                        ))
                    )}
                </div>
